# flows/charge_real/json/charge_real_add_sim.json

Source: lib/stac/tobank/flows/charge_real/json/charge_real_add_sim.json

## JSON Paths (sample)
- Could not parse JSON structure for path extraction.

## Raw JSON
```json
{
  "type": "stateFull",
  "onInit": {
    "actionType": "setValue",
    "values": [
      {
        "key": "crAddCanContinue",
        "value": false
      },
      {
        "key": "crAddEnteredPhone",
        "value": ""
      },
      {
        "key": "crAddIsValidPrefix",
        "value": false
      },
      {
        "key": "crAddIsUnsupported",
        "value": false
      },
      {
        "key": "crAddIsDuplicate",
        "value": false
      },
      {
        "key": "crAddIs0920",
        "value": false
      },
      {
        "key": "crAddIsMci",
        "value": false
      },
      {
        "key": "crAddCaseUnsupported",
        "value": false
      },
      {
        "key": "crAddCaseDuplicate",
        "value": false
      },
      {
        "key": "crAddCaseNormal",
        "value": false
      },
      {
        "key": "crAddCasePorted",
        "value": false
      },
      {
        "key": "crAddCaseInvalidAction",
        "value": false
      },
      {
        "key": "crAddShowInvalidError",
        "value": false
      },
      {
        "key": "crAddShowUnsupportedError",
        "value": false
      },
      {
        "key": "crAddShowDuplicateDialog",
        "value": false
      },
      {
        "key": "crAddShowOperatorSheet",
        "value": false
      },
      {
        "key": "crAddShowSimTypeSheet",
        "value": false
      },
      {
        "key": "crAddOpMciSel",
        "value": false
      },
      {
        "key": "crAddOpIrancellSel",
        "value": false
      },
      {
        "key": "crAddOpRightelSel",
        "value": true
      },
      {
        "key": "crAddTypePermanentSel",
        "value": true
      },
      {
        "key": "crAddTypeCreditSel",
        "value": false
      }
    ]
  },
  "child": {
    "appBar": {
      "leading": {
        "padding": {
          "left": 12.0
        },
        "child": {
          "child": {
            "src": "{{appAssets.icons.support}}",
            "imageType": "asset",
            "color": "{{appColors.current.text.title}}",
            "width": 24.0,
            "height": 24.0,
            "type": "image"
          },
          "type": "center"
        },
        "type": "padding"
      },
      "title": {
        "data": "Ø´Ø§Ø±Ú˜",
        "style": {
          "type": "alias",
          "value": "{{appStyles.appbarStyle}}"
        },
        "textDirection": "rtl",
        "type": "text"
      },
      "actions": [
        {
          "padding": {
            "right": 12.0
          },
          "child": {
            "onPressed": {
              "navigationStyle": "pop",
              "actionType": "navigate"
            },
            "icon": {
              "src": "{{appAssets.icons.arrowBack}}",
              "imageType": "asset",
              "color": "{{appColors.current.text.title}}",
              "width": 31.0,
              "height": 31.0,
              "type": "image"
            },
            "type": "iconButton"
          },
          "type": "padding"
        }
      ],
      "centerTitle": true,
      "type": "appBar"
    },
    "backgroundColor": "{{appColors.current.background.surface}}",
    "body": {
      "children": [
        {
          "top": false,
          "bottom": true,
          "child": {
            "autovalidateMode": "onUserInteraction",
            "child": {
              "children": [
                {
                  "child": {
                    "padding": {
                      "left": 16.0,
                      "top": 14.0,
                      "right": 16.0
                    },
                    "child": {
                      "crossAxisAlignment": "stretch",
                      "children": [
                        {
                          "data": "Ø´Ù…Ø§Ø±Ù‡ Ø³ÛŒÙ… Ú©Ø§Ø±Øª Ø§Ø¹ØªØ¨Ø§Ø±ÛŒ Ø±Ø§ ÙˆØ§Ø±Ø¯ Ú©Ù†ÛŒØ¯",
                          "style": {
                            "type": "custom",
                            "color": "{{appColors.current.text.title}}",
                            "fontSize": 18.0,
                            "fontWeight": "w700"
                          },
                          "textAlign": "right",
                          "textDirection": "rtl",
                          "type": "text"
                        },
                        {
                          "height": 18.0,
                          "type": "sizedBox"
                        },
                        {
                          "type": "registryReactive",
                          "child": {
                            "type": "textFormField",
                            "id": "crAddPhone",
                            "textDirection": "ltr",
                            "textAlign": "center",
                            "decoration": {
                              "hintText": "09123456789",
                              "hintStyle": {
                                "type": "custom",
                                "color": "#98A2B3",
                                "fontSize": 14.0,
                                "fontWeight": "w500"
                              },
                              "prefixIcon": {
                                "padding": {
                                  "left": 12.0,
                                  "right": 8.0
                                },
                                "child": {
                                  "child": {
                                    "src": "assets/icons/ic_pick_contact.svg",
                                    "imageType": "asset",
                                    "color": "{{appColors.current.text.subtitle}}",
                                    "width": 20.0,
                                    "height": 20.0,
                                    "type": "image"
                                  },
                                  "onTap": {
                                    "actionType": "pickContactPhone",
                                    "formFieldId": "crAddPhone",
                                    "targetKey": "crAddEnteredPhone",
                                    "permissionDeniedMessage": "Ø¯Ø³ØªØ±Ø³ÛŒ Ù…Ø®Ø§Ø·Ø¨ÛŒÙ† Ù…Ø¬Ø§Ø² Ù†ÛŒØ³Øª",
                                    "invalidMobileMessage": "Ø´Ù…Ø§Ø±Ù‡ Ù‡Ù…Ø±Ø§Ù‡ Ù…Ø¹ØªØ¨Ø± Ø¯Ø± Ù…Ø®Ø§Ø·Ø¨ ÛŒØ§ÙØª Ù†Ø´Ø¯",
                                    "onContactSelected": {
                                      "actionType": "sequence",
                                      "actions": [
                                        {
                                          "actionType": "setValue",
                                          "values": [
                                            {
                                              "key": "crAddShowInvalidError",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddShowUnsupportedError",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddCaseUnsupported",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddCaseDuplicate",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddCaseNormal",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddCasePorted",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddCaseInvalidAction",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddShowDuplicateDialog",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddShowOperatorSheet",
                                              "value": false
                                            },
                                            {
                                              "key": "crAddShowSimTypeSheet",
                                              "value": false
                                            }
                                          ]
                                        },
                                        {
                                          "actionType": "setValue",
                                          "key": "crAddEnteredPhone",
                                          "value": {
                                            "actionType": "getFormValue",
                                            "id": "crAddPhone"
                                          }
                                        },
                                        {
                                          "actionType": "validateFields",
                                          "resultKey": "crAddCanContinue",
                                          "fields": [
                                            {
                                              "id": "crAddPhone",
                                              "rule": "^09\\d{9}$"
                                            }
                                          ]
                                        },
                                        {
                                          "actionType": "validateFields",
                                          "resultKey": "crAddIsValidPrefix",
                                          "fields": [
                                            {
                                              "id": "crAddPhone",
                                              "rule": "^09\\d{9}$"
                                            }
                                          ]
                                        },
                                        {
                                          "actionType": "validateFields",
                                          "resultKey": "crAddIs0920",
                                          "fields": [
                                            {
                                              "id": "crAddPhone",
                                              "rule": "^0920\\d{7}$"
                                            }
                                          ]
                                        },
                                        {
                                          "actionType": "validateFields",
                                          "resultKey": "crAddIsMci",
                                          "fields": [
                                            {
                                              "id": "crAddPhone",
                                              "rule": "^091(0|2)\\d{7}$"
                                            }
                                          ]
                                        },
                                        {
                                          "actionType": "validateFields",
                                          "resultKey": "crAddIsUnsupported",
                                          "fields": [
                                            {
                                              "id": "crAddPhone",
                                              "rule": "^092[1-9]\\d{7}$"
                                            }
                                          ]
                                        },
                                        {
                                          "actionType": "validateFields",
                                          "resultKey": "crAddIsDuplicate",
                                          "fields": [
                                            {
                                              "id": "crAddPhone",
                                              "rule": "^(09124764369|09102311173)$"
                                            }
                                          ]
                                        },
                                        {
                                          "actionType": "setValue",
                                          "values": [
                                            {
                                              "key": "crAddCaseUnsupported",
                                              "value": true,
                                              "condition": "crAddIsUnsupported"
                                            },
                                            {
                                              "key": "crAddCaseDuplicate",
                                              "value": true,
                                              "condition": "crAddIsDuplicate"
                                            },
                                            {
                                              "key": "crAddCasePorted",
                                              "value": true,
                                              "condition": "crAddIs0920"
                                            },
                                            {
                                              "key": "crAddCaseNormal",
                                              "value": true,
                                              "condition": "crAddIsValidPrefix"
                                            },
                                            {
                                              "key": "crAddCaseNormal",
                                              "value": false,
                                              "condition": "crAddIsUnsupported"
                                            },
                                            {
                                              "key": "crAddCaseNormal",
                                              "value": false,
                                              "condition": "crAddIsDuplicate"
                                            },
                                            {
                                              "key": "crAddCaseNormal",
                                              "value": false,
                                              "condition": "crAddIs0920"
                                            },
                                            {
                                              "key": "crAddCaseInvalidAction",
                                              "value": true,
                                              "condition": "crAddCanContinue"
                                            },
                                            {
                                              "key": "crAddCaseInvalidAction",
                                              "value": false,
                                              "condition": "crAddCaseUnsupported"
                                            },
                                            {
                                              "key": "crAddCaseInvalidAction",
                                              "value": false,
                                              "condition": "crAddCaseDuplicate"
                                            },
                                            {
                                              "key": "crAddCaseInvalidAction",
                                              "value": false,
                                              "condition": "crAddCaseNormal"
                                            },
                                            {
                                              "key": "crAddCaseInvalidAction",
                                              "value": false,
                                              "condition": "crAddCasePorted"
                                            }
                                          ]
                                        }
                                      ]
                                    }
                                  },
                                  "type": "gestureDetector"
                                },
                                "type": "padding"
                              },
                              "contentPadding": {
                                "left": 12.0,
                                "top": 14.0,
                                "right": 12.0,
                                "bottom": 14.0
                              },
                              "filled": false,
                              "counterText": ""
                            },
                            "keyboardType": "number",
                            "textInputAction": "done",
                            "maxLength": 11,
                            "inputFormatters": [
                              {
                                "type": "allow",
                                "rule": "[0-9]"
                              }
                            ],
                            "onChanged": {
                              "actionType": "sequence",
                              "actions": [
                                {
                                  "actionType": "setValue",
                                  "values": [
                                    {
                                      "key": "crAddShowInvalidError",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddShowUnsupportedError",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddCaseUnsupported",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddCaseDuplicate",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddCaseNormal",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddCasePorted",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddCaseInvalidAction",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddShowDuplicateDialog",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddShowOperatorSheet",
                                      "value": false
                                    },
                                    {
                                      "key": "crAddShowSimTypeSheet",
                                      "value": false
                                    }
                                  ]
                                },
                                {
                                  "actionType": "setValue",
                                  "key": "crAddEnteredPhone",
                                  "value": {
                                    "actionType": "getFormValue",
                                    "id": "crAddPhone"
                                  }
                                },
                                {
                                  "actionType": "validateFields",
                                  "resultKey": "crAddCanContinue",
                                  "fields": [
                                    {
                                      "id": "crAddPhone",
                                      "rule": "^09\\d{9}$"
                                    }
                                  ]
                                },
                                {
                                  "actionType": "validateFields",
                                  "resultKey": "crAddIsValidPrefix",
                                  "fields": [
                                    {
                                      "id": "crAddPhone",
                                      "rule": "^09\\d{9}$"
                                    }
                                  ]
                                },
                                {
                                  "actionType": "validateFields",
                                  "resultKey": "crAddIs0920",
                                  "fields": [
                                    {
                                      "id": "crAddPhone",
                                      "rule": "^0920\\d{7}$"
                                    }
                                  ]
                                },
                                {
                                  "actionType": "validateFields",
                                  "resultKey": "crAddIsMci",
                                  "fields": [
                                    {
                                      "id": "crAddPhone",
                                      "rule": "^091(0|2)\\d{7}$"
                                    }
                                  ]
                                },
                                {
                                  "actionType": "validateFields",
                                  "resultKey": "crAddIsUnsupported",
                                  "fields": [
                                    {
                                      "id": "crAddPhone",
                                      "rule": "^092[1-9]\\d{7}$"
                                    }
                                  ]
                                },
                                {
                                  "actionType": "validateFields",
                                  "resultKey": "crAddIsDuplicate",
                                  "fields": [
                                    {
                                      "id": "crAddPhone",
                                      "rule": "^(09124764369|09102311173)$"
                                    }
                                  ]
                                },
                                {
                                  "actionType": "setValue",
                                  "values": [
                                    {
                                      "key": "crAddCaseUnsupported",
                                      "value": true,
                                      "condition": "crAddIsUnsupported"
                                    },
                                    {
                                      "key": "crAddCaseDuplicate",
                                      "value": true,
                                      "condition": "crAddIsDuplicate"
                                    },
                                    {
                                      "key": "crAddCasePorted",
                                      "value": true,
                                      "condition": "crAddIs0920"
                                    },
                                    {
                                      "key": "crAddCaseNormal",
                                      "value": true,
                                      "condition": "crAddIsValidPrefix"
                                    },
                                    {
                                      "key": "crAddCaseNormal",
                                      "value": false,
                                      "condition": "crAddIsUnsupported"
                                    },
                                    {
                                      "key": "crAddCaseNormal",
                                      "value": false,
                                      "condition": "crAddIsDuplicate"
                                    },
                                    {
                                      "key": "crAddCaseNormal",
                                      "value": false,
                                      "condition": "crAddIs0920"
                                    },
                                    {
                                      "key": "crAddCaseInvalidAction",
                                      "value": true,
                                      "condition": "crAddCanContinue"
                                    },
                                    {
                                      "key": "crAddCaseInvalidAction",
                                      "value": false,
                                      "condition": "crAddCaseUnsupported"
                                    },
                                    {
                                      "key": "crAddCaseInvalidAction",
                                      "value": false,
                                      "condition": "crAddCaseDuplicate"
                                    },
                                    {
                                      "key": "crAddCaseInvalidAction",
                                      "value": false,
                                      "condition": "crAddCaseNormal"
                                    },
                                    {
                                      "key": "crAddCaseInvalidAction",
                                      "value": false,
                                      "condition": "crAddCasePorted"
                                    }
                                  ]
                                }
                              ]
                            }
                          }
                        },
                        {
                          "type": "visibility",
                          "visible": "[[crAddShowInvalidError]]",
                          "child": {
                            "padding": {
                              "top": 8.0,
                              "right": 6.0
                            },
                            "child": {
                              "data": "Ø´Ù…Ø§Ø±Ù‡ Ù‡Ù…Ø±Ø§Ù‡ Ù†Ø§Ù…Ø¹ØªØ¨Ø± Ø§Ø³Øª",
                              "style": {
                                "type": "custom",
                                "color": "{{appColors.current.feedback.error}}",
                                "fontSize": 13.0,
                                "fontWeight": "w500"
                              },
                              "textAlign": "right",
                              "textDirection": "rtl",
                              "type": "text"
                            },
                            "type": "padding"
                          },
                          "replacement": {
                            "type": "sizedBox"
                          }
                        },
                        {
                          "type": "visibility",
                          "visible": "[[crAddShowUnsupportedError]]",
                          "child": {
                            "padding": {
                              "top": 8.0,
                              "right": 6.0
                            },
                            "child": {
                              "data": "Ø§Ù¾Ø±Ø§ØªÙˆØ± Ù…ÙˆØ±Ø¯ Ù†Ø¸Ø± Ù¾Ø´ØªÛŒØ¨Ø§Ù†ÛŒ Ù†Ù…ÛŒâ€ŒØ´ÙˆØ¯",
                              "style": {
                                "type": "custom",
                                "color": "{{appColors.current.feedback.error}}",
                                "fontSize": 13.0,
                                "fontWeight": "w500"
                              },
                              "textAlign": "right",
                              "textDirection": "rtl",
                              "type": "text"
                            },
                            "type": "padding"
                          },
                          "replacement": {
                            "type": "sizedBox"
                          }
                        }
                      ],
                      "type": "column"
                    },
                    "type": "singleChildScrollView"
                  },
                  "type": "expanded"
                },
                {
                  "type": "visibility",
                  "visible": "[[!crAddCanContinue]]",
                  "child": {
                    "padding": {
                      "left": 16.0,
                      "right": 16.0,
                      "bottom": 18.0
                    },
                    "child": {
                      "style": {
                        "foregroundColor": "#98A2B3",
                        "backgroundColor": "#D0D5DD",
                        "elevation": 0.0,
                        "fixedSize": {
                          "width": 99999.0,
                          "height": 56.0
                        },
                        "shape": {
                          "type": "roundedRectangleBorder",
                          "borderRadius": {
                            "topLeft": 10.0,
                            "topRight": 10.0,
                            "bottomLeft": 10.0,
                            "bottomRight": 10.0
                          }
                        }
                      },
                      "child": {
                        "data": "Ø§Ø¯Ø§Ù…Ù‡",
                        "style": {
                          "type": "custom",
                          "color": "#98A2B3",
                          "fontSize": 16.0,
                          "fontWeight": "w700"
                        },
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      "type": "filledButton"
                    },
                    "type": "padding"
                  },
                  "replacement": {
                    "type": "sizedBox"
                  }
                },
                {
                  "type": "visibility",
                  "visible": "[[crAddCaseNormal]]",
                  "child": {
                    "padding": {
                      "left": 16.0,
                      "right": 16.0,
                      "bottom": 18.0
                    },
                    "child": {
                      "type": "reactiveElevatedButton",
                      "enabled": true,
                      "style": {
                        "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                        "backgroundColor": "{{appColors.current.primary.color}}",
                        "elevation": 0.0,
                        "fixedSize": {
                          "width": 99999.0,
                          "height": 56.0
                        },
                        "shape": {
                          "type": "roundedRectangleBorder",
                          "borderRadius": {
                            "topLeft": 10.0,
                            "topRight": 10.0,
                            "bottomLeft": 10.0,
                            "bottomRight": 10.0
                          }
                        }
                      },
                      "child": {
                        "data": "Ø§Ø¯Ø§Ù…Ù‡",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.primary.onPrimary}}",
                          "fontSize": 16.0,
                          "fontWeight": "w700"
                        },
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      "onPressed": {
                        "actionType": "sequence",
                        "actions": [
                          {
                            "actionType": "setValue",
                            "values": [
                              {
                                "key": "crSimHasItems",
                                "value": true
                              },
                              {
                                "key": "crSimShowDuplicateBanner",
                                "value": false
                              },
                              {
                                "key": "crSim3Visible",
                                "value": true
                              },
                              {
                                "key": "crSim3Operator",
                                "value": "Ø§ÛŒØ±Ø§Ù†Ø³Ù„"
                              },
                              {
                                "key": "crSim3Logo",
                                "value": "assets/icons/ic_irancell.svg"
                              },
                              {
                                "key": "crSim1Selected",
                                "value": false
                              },
                              {
                                "key": "crSim2Selected",
                                "value": false
                              },
                              {
                                "key": "crSim3Selected",
                                "value": true
                              },
                              {
                                "key": "crActiveSimOperator",
                                "value": "Ø§ÛŒØ±Ø§Ù†Ø³Ù„"
                              },
                              {
                                "key": "crActiveSimLogo",
                                "value": "assets/icons/ic_irancell.svg"
                              },
                              {
                                "key": "crSim3Operator",
                                "value": "Ù‡Ù…Ø±Ø§Ù‡ Ø§ÙˆÙ„",
                                "condition": "crAddIsMci"
                              },
                              {
                                "key": "crSim3Logo",
                                "value": "assets/icons/ic_hamrah_aval.svg",
                                "condition": "crAddIsMci"
                              },
                              {
                                "key": "crActiveSimOperator",
                                "value": "Ù‡Ù…Ø±Ø§Ù‡ Ø§ÙˆÙ„",
                                "condition": "crAddIsMci"
                              },
                              {
                                "key": "crActiveSimLogo",
                                "value": "assets/icons/ic_hamrah_aval.svg",
                                "condition": "crAddIsMci"
                              },
                              {
                                "key": "crShowSimActionSheet",
                                "value": false
                              },
                              {
                                "key": "crShowSimEditSheet",
                                "value": false
                              },
                              {
                                "key": "crShowSimDeleteDialog",
                                "value": false
                              },
                              {
                                "key": "crAddShowInvalidError",
                                "value": false
                              },
                              {
                                "key": "crAddShowUnsupportedError",
                                "value": false
                              }
                            ]
                          },
                          {
                            "actionType": "setValue",
                            "key": "crSim3Number",
                            "value": "__STAC_OPEN__crAddEnteredPhone}}"
                          },
                          {
                            "actionType": "setValue",
                            "key": "crActiveSimNumber",
                            "value": "__STAC_OPEN__crAddEnteredPhone}}"
                          },
                          {
                            "routeName": "charge_real_package_list",
                            "navigationStyle": "push",
                            "actionType": "navigate"
                          }
                        ]
                      }
                    },
                    "type": "padding"
                  },
                  "replacement": {
                    "type": "sizedBox"
                  }
                },
                {
                  "type": "visibility",
                  "visible": "[[crAddCaseDuplicate]]",
                  "child": {
                    "padding": {
                      "left": 16.0,
                      "right": 16.0,
                      "bottom": 18.0
                    },
                    "child": {
                      "type": "reactiveElevatedButton",
                      "enabled": true,
                      "style": {
                        "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                        "backgroundColor": "{{appColors.current.primary.color}}",
                        "elevation": 0.0,
                        "fixedSize": {
                          "width": 99999.0,
                          "height": 56.0
                        },
                        "shape": {
                          "type": "roundedRectangleBorder",
                          "borderRadius": {
                            "topLeft": 10.0,
                            "topRight": 10.0,
                            "bottomLeft": 10.0,
                            "bottomRight": 10.0
                          }
                        }
                      },
                      "child": {
                        "data": "Ø§Ø¯Ø§Ù…Ù‡",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.primary.onPrimary}}",
                          "fontSize": 16.0,
                          "fontWeight": "w700"
                        },
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      "onPressed": {
                        "actionType": "setValue",
                        "values": [
                          {
                            "key": "crAddShowDuplicateDialog",
                            "value": true
                          },
                          {
                            "key": "crAddShowInvalidError",
                            "value": false
                          },
                          {
                            "key": "crAddShowUnsupportedError",
                            "value": false
                          }
                        ]
                      }
                    },
                    "type": "padding"
                  },
                  "replacement": {
                    "type": "sizedBox"
                  }
                },
                {
                  "type": "visibility",
                  "visible": "[[crAddCasePorted]]",
                  "child": {
                    "padding": {
                      "left": 16.0,
                      "right": 16.0,
                      "bottom": 18.0
                    },
                    "child": {
                      "type": "reactiveElevatedButton",
                      "enabled": true,
                      "style": {
                        "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                        "backgroundColor": "{{appColors.current.primary.color}}",
                        "elevation": 0.0,
                        "fixedSize": {
                          "width": 99999.0,
                          "height": 56.0
                        },
                        "shape": {
                          "type": "roundedRectangleBorder",
                          "borderRadius": {
                            "topLeft": 10.0,
                            "topRight": 10.0,
                            "bottomLeft": 10.0,
                            "bottomRight": 10.0
                          }
                        }
                      },
                      "child": {
                        "data": "Ø§Ø¯Ø§Ù…Ù‡",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.primary.onPrimary}}",
                          "fontSize": 16.0,
                          "fontWeight": "w700"
                        },
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      "onPressed": {
                        "actionType": "setValue",
                        "values": [
                          {
                            "key": "crAddShowOperatorSheet",
                            "value": true
                          },
                          {
                            "key": "crAddShowSimTypeSheet",
                            "value": false
                          }
                        ]
                      }
                    },
                    "type": "padding"
                  },
                  "replacement": {
                    "type": "sizedBox"
                  }
                },
                {
                  "type": "visibility",
                  "visible": "[[crAddCaseUnsupported]]",
                  "child": {
                    "padding": {
                      "left": 16.0,
                      "right": 16.0,
                      "bottom": 18.0
                    },
                    "child": {
                      "type": "reactiveElevatedButton",
                      "enabled": true,
                      "style": {
                        "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                        "backgroundColor": "{{appColors.current.primary.color}}",
                        "elevation": 0.0,
                        "fixedSize": {
                          "width": 99999.0,
                          "height": 56.0
                        },
                        "shape": {
                          "type": "roundedRectangleBorder",
                          "borderRadius": {
                            "topLeft": 10.0,
                            "topRight": 10.0,
                            "bottomLeft": 10.0,
                            "bottomRight": 10.0
                          }
                        }
                      },
                      "child": {
                        "data": "Ø§Ø¯Ø§Ù…Ù‡",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.primary.onPrimary}}",
                          "fontSize": 16.0,
                          "fontWeight": "w700"
                        },
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      "onPressed": {
                        "actionType": "setValue",
                        "values": [
                          {
                            "key": "crAddShowInvalidError",
                            "value": false
                          },
                          {
                            "key": "crAddShowUnsupportedError",
                            "value": true
                          }
                        ]
                      }
                    },
                    "type": "padding"
                  },
                  "replacement": {
                    "type": "sizedBox"
                  }
                },
                {
                  "type": "visibility",
                  "visible": "[[crAddCaseInvalidAction]]",
                  "child": {
                    "padding": {
                      "left": 16.0,
                      "right": 16.0,
                      "bottom": 18.0
                    },
                    "child": {
                      "type": "reactiveElevatedButton",
                      "enabled": true,
                      "style": {
                        "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                        "backgroundColor": "{{appColors.current.primary.color}}",
                        "elevation": 0.0,
                        "fixedSize": {
                          "width": 99999.0,
                          "height": 56.0
                        },
                        "shape": {
                          "type": "roundedRectangleBorder",
                          "borderRadius": {
                            "topLeft": 10.0,
                            "topRight": 10.0,
                            "bottomLeft": 10.0,
                            "bottomRight": 10.0
                          }
                        }
                      },
                      "child": {
                        "data": "Ø§Ø¯Ø§Ù…Ù‡",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.primary.onPrimary}}",
                          "fontSize": 16.0,
                          "fontWeight": "w700"
                        },
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      "onPressed": {
                        "actionType": "setValue",
                        "values": [
                          {
                            "key": "crAddShowInvalidError",
                            "value": true
                          },
                          {
                            "key": "crAddShowUnsupportedError",
                            "value": false
                          }
                        ]
                      }
                    },
                    "type": "padding"
                  },
                  "replacement": {
                    "type": "sizedBox"
                  }
                }
              ],
              "type": "column"
            },
            "type": "form"
          },
          "type": "safeArea"
        },
        {
          "type": "visibility",
          "visible": "[[crAddShowDuplicateDialog]]",
          "child": {
            "children": [
              {
                "color": "#8B63708C",
                "width": 999999.0,
                "height": 999999.0,
                "type": "container"
              },
              {
                "child": {
                  "padding": {
                    "left": 14.0,
                    "top": 16.0,
                    "right": 14.0,
                    "bottom": 12.0
                  },
                  "decoration": {
                    "color": "{{appColors.current.background.surface}}",
                    "border": {
                      "color": "#2B8BFA",
                      "width": 2.0
                    },
                    "borderRadius": {
                      "topLeft": 12.0,
                      "topRight": 12.0,
                      "bottomLeft": 12.0,
                      "bottomRight": 12.0
                    }
                  },
                  "margin": {
                    "left": 16.0,
                    "right": 16.0
                  },
                  "child": {
                    "mainAxisSize": "min",
                    "crossAxisAlignment": "stretch",
                    "children": [
                      {
                        "child": {
                          "decoration": {
                            "color": "#FFCD29",
                            "shape": "circle"
                          },
                          "width": 28.0,
                          "height": 28.0,
                          "child": {
                            "child": {
                              "icon": "error_outline",
                              "iconType": "material",
                              "size": 18.0,
                              "color": "#FFFFFF",
                              "type": "icon"
                            },
                            "type": "center"
                          },
                          "type": "container"
                        },
                        "type": "center"
                      },
                      {
                        "height": 10.0,
                        "type": "sizedBox"
                      },
                      {
                        "data": "Ù…Ø¬Ø§Ø² Ù†ÛŒØ³Øª",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.text.title}}",
                          "fontSize": 18.0,
                          "fontWeight": "w700"
                        },
                        "textAlign": "center",
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      {
                        "height": 10.0,
                        "type": "sizedBox"
                      },
                      {
                        "data": "Ø´Ù…Ø§Ø±Ù‡ Ø§Ù†ØªØ®Ø§Ø¨ÛŒ Ø´Ù…Ø§ Ø¬Ø² Ø³ÛŒÙ…â€ŒÚ©Ø§Ø±Øªâ€ŒÙ‡Ø§ÛŒ ØªÚ©Ø±Ø§Ø±ÛŒ Ù…ÛŒâ€ŒØ¨Ø§Ø´Ø¯ØŒ Ù„Ø·ÙØ§ ÛŒÚ©ÛŒ Ø¯ÛŒÚ¯Ø± Ø§Ù†ØªØ®Ø§Ø¨ Ú©Ù†ÛŒØ¯.",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.text.subtitle}}",
                          "fontSize": 13.0,
                          "fontWeight": "w500"
                        },
                        "textAlign": "center",
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      {
                        "height": 14.0,
                        "type": "sizedBox"
                      },
                      {
                        "onPressed": {
                          "actionType": "setValue",
                          "key": "crAddShowDuplicateDialog",
                          "value": false
                        },
                        "style": {
                          "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                          "backgroundColor": "{{appColors.current.primary.color}}",
                          "elevation": 0.0,
                          "fixedSize": {
                            "width": 999999.0,
                            "height": 44.0
                          },
                          "shape": {
                            "type": "roundedRectangleBorder",
                            "borderRadius": {
                              "topLeft": 8.0,
                              "topRight": 8.0,
                              "bottomLeft": 8.0,
                              "bottomRight": 8.0
                            }
                          }
                        },
                        "child": {
                          "data": "Ù…ØªÙˆØ¬Ù‡ Ø´Ø¯Ù…",
                          "style": {
                            "type": "custom",
                            "color": "{{appColors.current.primary.onPrimary}}",
                            "fontSize": 14.0,
                            "fontWeight": "w700"
                          },
                          "textDirection": "rtl",
                          "type": "text"
                        },
                        "type": "filledButton"
                      }
                    ],
                    "type": "column"
                  },
                  "type": "container"
                },
                "type": "center"
              }
            ],
            "type": "stack"
          },
          "replacement": {
            "type": "sizedBox"
          }
        },
        {
          "type": "visibility",
          "visible": "[[crAddShowOperatorSheet]]",
          "child": {
            "children": [
              {
                "color": "#8B63708C",
                "width": 999999.0,
                "height": 999999.0,
                "type": "container"
              },
              {
                "alignment": "bottomCenter",
                "child": {
                  "padding": {
                    "left": 14.0,
                    "top": 10.0,
                    "right": 14.0,
                    "bottom": 16.0
                  },
                  "decoration": {
                    "color": "{{appColors.current.background.surface}}",
                    "borderRadius": {
                      "topLeft": 22.0,
                      "topRight": 22.0
                    }
                  },
                  "width": 999999.0,
                  "child": {
                    "mainAxisSize": "min",
                    "crossAxisAlignment": "stretch",
                    "children": [
                      {
                        "child": {
                          "decoration": {
                            "color": "#D0D5DD",
                            "borderRadius": {
                              "topLeft": 99.0,
                              "topRight": 99.0,
                              "bottomLeft": 99.0,
                              "bottomRight": 99.0
                            }
                          },
                          "width": 44.0,
                          "height": 5.0,
                          "type": "container"
                        },
                        "type": "center"
                      },
                      {
                        "height": 16.0,
                        "type": "sizedBox"
                      },
                      {
                        "data": "Ø§Ù¾Ø±Ø§ØªÙˆØ±",
                        "style": {
                          "type": "custom",
                          "fontSize": 18.0,
                          "fontWeight": "w700"
                        },
                        "textAlign": "center",
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      {
                        "height": 8.0,
                        "type": "sizedBox"
                      },
                      {
                        "data": "Ø¯Ø± ØµÙˆØ±Øª ØªØ±Ø§Ø¨Ø±Ø¯ Ø³ÛŒÙ…â€ŒÚ©Ø§Ø±Øª Ø§Ù¾Ø±Ø§ØªÙˆØ± Ø®ÙˆØ¯ Ø±Ø§ Ø§Ù†ØªØ®Ø§Ø¨ Ù†Ù…Ø§ÛŒÛŒØ¯",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.text.subtitle}}",
                          "fontSize": 13.0
                        },
                        "textAlign": "center",
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      {
                        "height": 14.0,
                        "type": "sizedBox"
                      },
                      {
                        "child": {
                          "type": "visibility",
                          "visible": "[[crAddOpMciSel]]",
                          "child": {
                            "decoration": {
                              "border": {
                                "color": "#20C4D8",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "padding": {
                                "left": 12.0,
                                "top": 12.0,
                                "right": 12.0,
                                "bottom": 12.0
                              },
                              "decoration": {
                                "border": {
                                  "color": "{{appColors.current.input.borderEnabled}}",
                                  "width": 1.0
                                },
                                "borderRadius": {
                                  "topLeft": 10.0,
                                  "topRight": 10.0,
                                  "bottomLeft": 10.0,
                                  "bottomRight": 10.0
                                }
                              },
                              "child": {
                                "textDirection": "rtl",
                                "children": [
                                  {
                                    "src": "assets/icons/ic_hamrah_aval.svg",
                                    "imageType": "asset",
                                    "width": 20.0,
                                    "height": 20.0,
                                    "type": "image"
                                  },
                                  {
                                    "width": 10.0,
                                    "type": "sizedBox"
                                  },
                                  {
                                    "data": "Ù‡Ù…Ø±Ø§Ù‡ Ø§ÙˆÙ„",
                                    "style": {
                                      "type": "custom",
                                      "fontSize": 18.0,
                                      "fontWeight": "w500"
                                    },
                                    "textDirection": "rtl",
                                    "type": "text"
                                  },
                                  {
                                    "child": {
                                      "type": "sizedBox"
                                    },
                                    "type": "expanded"
                                  },
                                  {
                                    "type": "visibility",
                                    "visible": "[[crAddOpMciSel]]",
                                    "child": {
                                      "decoration": {
                                        "color": "#20C4D8",
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "child": {
                                        "child": {
                                          "icon": "check",
                                          "iconType": "material",
                                          "size": 16.0,
                                          "color": "#FFFFFF",
                                          "type": "icon"
                                        },
                                        "type": "center"
                                      },
                                      "type": "container"
                                    },
                                    "replacement": {
                                      "decoration": {
                                        "border": {
                                          "color": "{{appColors.current.input.borderEnabled}}",
                                          "width": 1.0
                                        },
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "type": "container"
                                    }
                                  }
                                ],
                                "type": "row"
                              },
                              "type": "container"
                            },
                            "type": "container"
                          },
                          "replacement": {
                            "padding": {
                              "left": 12.0,
                              "top": 12.0,
                              "right": 12.0,
                              "bottom": 12.0
                            },
                            "decoration": {
                              "border": {
                                "color": "{{appColors.current.input.borderEnabled}}",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "textDirection": "rtl",
                              "children": [
                                {
                                  "src": "assets/icons/ic_hamrah_aval.svg",
                                  "imageType": "asset",
                                  "width": 20.0,
                                  "height": 20.0,
                                  "type": "image"
                                },
                                {
                                  "width": 10.0,
                                  "type": "sizedBox"
                                },
                                {
                                  "data": "Ù‡Ù…Ø±Ø§Ù‡ Ø§ÙˆÙ„",
                                  "style": {
                                    "type": "custom",
                                    "fontSize": 18.0,
                                    "fontWeight": "w500"
                                  },
                                  "textDirection": "rtl",
                                  "type": "text"
                                },
                                {
                                  "child": {
                                    "type": "sizedBox"
                                  },
                                  "type": "expanded"
                                },
                                {
                                  "type": "visibility",
                                  "visible": "[[crAddOpMciSel]]",
                                  "child": {
                                    "decoration": {
                                      "color": "#20C4D8",
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "child": {
                                      "child": {
                                        "icon": "check",
                                        "iconType": "material",
                                        "size": 16.0,
                                        "color": "#FFFFFF",
                                        "type": "icon"
                                      },
                                      "type": "center"
                                    },
                                    "type": "container"
                                  },
                                  "replacement": {
                                    "decoration": {
                                      "border": {
                                        "color": "{{appColors.current.input.borderEnabled}}",
                                        "width": 1.0
                                      },
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "type": "container"
                                  }
                                }
                              ],
                              "type": "row"
                            },
                            "type": "container"
                          }
                        },
                        "onTap": {
                          "actionType": "setValue",
                          "values": [
                            {
                              "key": "crAddOpMciSel",
                              "value": true
                            },
                            {
                              "key": "crAddOpIrancellSel",
                              "value": false
                            },
                            {
                              "key": "crAddOpRightelSel",
                              "value": false
                            }
                          ]
                        },
                        "type": "gestureDetector"
                      },
                      {
                        "height": 10.0,
                        "type": "sizedBox"
                      },
                      {
                        "child": {
                          "type": "visibility",
                          "visible": "[[crAddOpIrancellSel]]",
                          "child": {
                            "decoration": {
                              "border": {
                                "color": "#20C4D8",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "padding": {
                                "left": 12.0,
                                "top": 12.0,
                                "right": 12.0,
                                "bottom": 12.0
                              },
                              "decoration": {
                                "border": {
                                  "color": "{{appColors.current.input.borderEnabled}}",
                                  "width": 1.0
                                },
                                "borderRadius": {
                                  "topLeft": 10.0,
                                  "topRight": 10.0,
                                  "bottomLeft": 10.0,
                                  "bottomRight": 10.0
                                }
                              },
                              "child": {
                                "textDirection": "rtl",
                                "children": [
                                  {
                                    "src": "assets/icons/ic_irancell.svg",
                                    "imageType": "asset",
                                    "width": 20.0,
                                    "height": 20.0,
                                    "type": "image"
                                  },
                                  {
                                    "width": 10.0,
                                    "type": "sizedBox"
                                  },
                                  {
                                    "data": "Ø§ÛŒØ±Ø§Ù†Ø³Ù„",
                                    "style": {
                                      "type": "custom",
                                      "fontSize": 18.0,
                                      "fontWeight": "w500"
                                    },
                                    "textDirection": "rtl",
                                    "type": "text"
                                  },
                                  {
                                    "child": {
                                      "type": "sizedBox"
                                    },
                                    "type": "expanded"
                                  },
                                  {
                                    "type": "visibility",
                                    "visible": "[[crAddOpIrancellSel]]",
                                    "child": {
                                      "decoration": {
                                        "color": "#20C4D8",
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "child": {
                                        "child": {
                                          "icon": "check",
                                          "iconType": "material",
                                          "size": 16.0,
                                          "color": "#FFFFFF",
                                          "type": "icon"
                                        },
                                        "type": "center"
                                      },
                                      "type": "container"
                                    },
                                    "replacement": {
                                      "decoration": {
                                        "border": {
                                          "color": "{{appColors.current.input.borderEnabled}}",
                                          "width": 1.0
                                        },
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "type": "container"
                                    }
                                  }
                                ],
                                "type": "row"
                              },
                              "type": "container"
                            },
                            "type": "container"
                          },
                          "replacement": {
                            "padding": {
                              "left": 12.0,
                              "top": 12.0,
                              "right": 12.0,
                              "bottom": 12.0
                            },
                            "decoration": {
                              "border": {
                                "color": "{{appColors.current.input.borderEnabled}}",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "textDirection": "rtl",
                              "children": [
                                {
                                  "src": "assets/icons/ic_irancell.svg",
                                  "imageType": "asset",
                                  "width": 20.0,
                                  "height": 20.0,
                                  "type": "image"
                                },
                                {
                                  "width": 10.0,
                                  "type": "sizedBox"
                                },
                                {
                                  "data": "Ø§ÛŒØ±Ø§Ù†Ø³Ù„",
                                  "style": {
                                    "type": "custom",
                                    "fontSize": 18.0,
                                    "fontWeight": "w500"
                                  },
                                  "textDirection": "rtl",
                                  "type": "text"
                                },
                                {
                                  "child": {
                                    "type": "sizedBox"
                                  },
                                  "type": "expanded"
                                },
                                {
                                  "type": "visibility",
                                  "visible": "[[crAddOpIrancellSel]]",
                                  "child": {
                                    "decoration": {
                                      "color": "#20C4D8",
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "child": {
                                      "child": {
                                        "icon": "check",
                                        "iconType": "material",
                                        "size": 16.0,
                                        "color": "#FFFFFF",
                                        "type": "icon"
                                      },
                                      "type": "center"
                                    },
                                    "type": "container"
                                  },
                                  "replacement": {
                                    "decoration": {
                                      "border": {
                                        "color": "{{appColors.current.input.borderEnabled}}",
                                        "width": 1.0
                                      },
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "type": "container"
                                  }
                                }
                              ],
                              "type": "row"
                            },
                            "type": "container"
                          }
                        },
                        "onTap": {
                          "actionType": "setValue",
                          "values": [
                            {
                              "key": "crAddOpMciSel",
                              "value": false
                            },
                            {
                              "key": "crAddOpIrancellSel",
                              "value": true
                            },
                            {
                              "key": "crAddOpRightelSel",
                              "value": false
                            }
                          ]
                        },
                        "type": "gestureDetector"
                      },
                      {
                        "height": 10.0,
                        "type": "sizedBox"
                      },
                      {
                        "child": {
                          "type": "visibility",
                          "visible": "[[crAddOpRightelSel]]",
                          "child": {
                            "decoration": {
                              "border": {
                                "color": "#20C4D8",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "padding": {
                                "left": 12.0,
                                "top": 12.0,
                                "right": 12.0,
                                "bottom": 12.0
                              },
                              "decoration": {
                                "border": {
                                  "color": "{{appColors.current.input.borderEnabled}}",
                                  "width": 1.0
                                },
                                "borderRadius": {
                                  "topLeft": 10.0,
                                  "topRight": 10.0,
                                  "bottomLeft": 10.0,
                                  "bottomRight": 10.0
                                }
                              },
                              "child": {
                                "textDirection": "rtl",
                                "children": [
                                  {
                                    "src": "assets/icons/ic_rightel.svg",
                                    "imageType": "asset",
                                    "width": 20.0,
                                    "height": 20.0,
                                    "type": "image"
                                  },
                                  {
                                    "width": 10.0,
                                    "type": "sizedBox"
                                  },
                                  {
                                    "data": "Ø±Ø§ÛŒØªÙ„",
                                    "style": {
                                      "type": "custom",
                                      "fontSize": 18.0,
                                      "fontWeight": "w500"
                                    },
                                    "textDirection": "rtl",
                                    "type": "text"
                                  },
                                  {
                                    "child": {
                                      "type": "sizedBox"
                                    },
                                    "type": "expanded"
                                  },
                                  {
                                    "type": "visibility",
                                    "visible": "[[crAddOpRightelSel]]",
                                    "child": {
                                      "decoration": {
                                        "color": "#20C4D8",
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "child": {
                                        "child": {
                                          "icon": "check",
                                          "iconType": "material",
                                          "size": 16.0,
                                          "color": "#FFFFFF",
                                          "type": "icon"
                                        },
                                        "type": "center"
                                      },
                                      "type": "container"
                                    },
                                    "replacement": {
                                      "decoration": {
                                        "border": {
                                          "color": "{{appColors.current.input.borderEnabled}}",
                                          "width": 1.0
                                        },
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "type": "container"
                                    }
                                  }
                                ],
                                "type": "row"
                              },
                              "type": "container"
                            },
                            "type": "container"
                          },
                          "replacement": {
                            "padding": {
                              "left": 12.0,
                              "top": 12.0,
                              "right": 12.0,
                              "bottom": 12.0
                            },
                            "decoration": {
                              "border": {
                                "color": "{{appColors.current.input.borderEnabled}}",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "textDirection": "rtl",
                              "children": [
                                {
                                  "src": "assets/icons/ic_rightel.svg",
                                  "imageType": "asset",
                                  "width": 20.0,
                                  "height": 20.0,
                                  "type": "image"
                                },
                                {
                                  "width": 10.0,
                                  "type": "sizedBox"
                                },
                                {
                                  "data": "Ø±Ø§ÛŒØªÙ„",
                                  "style": {
                                    "type": "custom",
                                    "fontSize": 18.0,
                                    "fontWeight": "w500"
                                  },
                                  "textDirection": "rtl",
                                  "type": "text"
                                },
                                {
                                  "child": {
                                    "type": "sizedBox"
                                  },
                                  "type": "expanded"
                                },
                                {
                                  "type": "visibility",
                                  "visible": "[[crAddOpRightelSel]]",
                                  "child": {
                                    "decoration": {
                                      "color": "#20C4D8",
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "child": {
                                      "child": {
                                        "icon": "check",
                                        "iconType": "material",
                                        "size": 16.0,
                                        "color": "#FFFFFF",
                                        "type": "icon"
                                      },
                                      "type": "center"
                                    },
                                    "type": "container"
                                  },
                                  "replacement": {
                                    "decoration": {
                                      "border": {
                                        "color": "{{appColors.current.input.borderEnabled}}",
                                        "width": 1.0
                                      },
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "type": "container"
                                  }
                                }
                              ],
                              "type": "row"
                            },
                            "type": "container"
                          }
                        },
                        "onTap": {
                          "actionType": "setValue",
                          "values": [
                            {
                              "key": "crAddOpMciSel",
                              "value": false
                            },
                            {
                              "key": "crAddOpIrancellSel",
                              "value": false
                            },
                            {
                              "key": "crAddOpRightelSel",
                              "value": true
                            }
                          ]
                        },
                        "type": "gestureDetector"
                      },
                      {
                        "height": 14.0,
                        "type": "sizedBox"
                      },
                      {
                        "onPressed": {
                          "actionType": "setValue",
                          "values": [
                            {
                              "key": "crAddShowOperatorSheet",
                              "value": false
                            },
                            {
                              "key": "crAddShowSimTypeSheet",
                              "value": true
                            }
                          ]
                        },
                        "style": {
                          "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                          "backgroundColor": "{{appColors.current.primary.color}}",
                          "elevation": 0.0,
                          "fixedSize": {
                            "width": 999999.0,
                            "height": 56.0
                          },
                          "shape": {
                            "type": "roundedRectangleBorder",
                            "borderRadius": {
                              "topLeft": 10.0,
                              "topRight": 10.0,
                              "bottomLeft": 10.0,
                              "bottomRight": 10.0
                            }
                          }
                        },
                        "child": {
                          "data": "ØªØ§ÛŒÛŒØ¯ Ùˆ Ø§Ø¯Ø§Ù…Ù‡",
                          "style": {
                            "type": "custom",
                            "color": "{{appColors.current.primary.onPrimary}}",
                            "fontSize": 16.0,
                            "fontWeight": "w700"
                          },
                          "textDirection": "rtl",
                          "type": "text"
                        },
                        "type": "filledButton"
                      }
                    ],
                    "type": "column"
                  },
                  "type": "container"
                },
                "type": "align"
              }
            ],
            "type": "stack"
          },
          "replacement": {
            "type": "sizedBox"
          }
        },
        {
          "type": "visibility",
          "visible": "[[crAddShowSimTypeSheet]]",
          "child": {
            "children": [
              {
                "color": "#8B63708C",
                "width": 999999.0,
                "height": 999999.0,
                "type": "container"
              },
              {
                "alignment": "bottomCenter",
                "child": {
                  "padding": {
                    "left": 14.0,
                    "top": 10.0,
                    "right": 14.0,
                    "bottom": 16.0
                  },
                  "decoration": {
                    "color": "{{appColors.current.background.surface}}",
                    "borderRadius": {
                      "topLeft": 22.0,
                      "topRight": 22.0
                    }
                  },
                  "width": 999999.0,
                  "child": {
                    "mainAxisSize": "min",
                    "crossAxisAlignment": "stretch",
                    "children": [
                      {
                        "child": {
                          "decoration": {
                            "color": "#D0D5DD",
                            "borderRadius": {
                              "topLeft": 99.0,
                              "topRight": 99.0,
                              "bottomLeft": 99.0,
                              "bottomRight": 99.0
                            }
                          },
                          "width": 44.0,
                          "height": 5.0,
                          "type": "container"
                        },
                        "type": "center"
                      },
                      {
                        "height": 16.0,
                        "type": "sizedBox"
                      },
                      {
                        "data": "Ù†ÙˆØ¹ Ø³ÛŒÙ… Ú©Ø§Ø±Øª",
                        "style": {
                          "type": "custom",
                          "fontSize": 18.0,
                          "fontWeight": "w700"
                        },
                        "textAlign": "center",
                        "textDirection": "rtl",
                        "type": "text"
                      },
                      {
                        "height": 8.0,
                        "type": "sizedBox"
                      },
                      {
                        "data": "{{crAddEnteredPhone}}",
                        "style": {
                          "type": "custom",
                          "color": "{{appColors.current.text.subtitle}}",
                          "fontSize": 16.0
                        },
                        "textAlign": "center",
                        "textDirection": "ltr",
                        "type": "text"
                      },
                      {
                        "height": 14.0,
                        "type": "sizedBox"
                      },
                      {
                        "child": {
                          "type": "visibility",
                          "visible": "[[crAddTypePermanentSel]]",
                          "child": {
                            "decoration": {
                              "border": {
                                "color": "#20C4D8",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "padding": {
                                "left": 12.0,
                                "top": 14.0,
                                "right": 12.0,
                                "bottom": 14.0
                              },
                              "decoration": {
                                "border": {
                                  "color": "{{appColors.current.input.borderEnabled}}",
                                  "width": 1.0
                                },
                                "borderRadius": {
                                  "topLeft": 10.0,
                                  "topRight": 10.0,
                                  "bottomLeft": 10.0,
                                  "bottomRight": 10.0
                                }
                              },
                              "child": {
                                "textDirection": "rtl",
                                "children": [
                                  {
                                    "data": "Ø¯Ø§Ø¦Ù…ÛŒ",
                                    "style": {
                                      "type": "custom",
                                      "fontSize": 16.0,
                                      "fontWeight": "w500"
                                    },
                                    "textDirection": "rtl",
                                    "type": "text"
                                  },
                                  {
                                    "child": {
                                      "type": "sizedBox"
                                    },
                                    "type": "expanded"
                                  },
                                  {
                                    "type": "visibility",
                                    "visible": "[[crAddTypePermanentSel]]",
                                    "child": {
                                      "decoration": {
                                        "color": "#20C4D8",
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "child": {
                                        "child": {
                                          "icon": "check",
                                          "iconType": "material",
                                          "size": 16.0,
                                          "color": "#FFFFFF",
                                          "type": "icon"
                                        },
                                        "type": "center"
                                      },
                                      "type": "container"
                                    },
                                    "replacement": {
                                      "decoration": {
                                        "border": {
                                          "color": "{{appColors.current.input.borderEnabled}}",
                                          "width": 1.0
                                        },
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "type": "container"
                                    }
                                  }
                                ],
                                "type": "row"
                              },
                              "type": "container"
                            },
                            "type": "container"
                          },
                          "replacement": {
                            "padding": {
                              "left": 12.0,
                              "top": 14.0,
                              "right": 12.0,
                              "bottom": 14.0
                            },
                            "decoration": {
                              "border": {
                                "color": "{{appColors.current.input.borderEnabled}}",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "textDirection": "rtl",
                              "children": [
                                {
                                  "data": "Ø¯Ø§Ø¦Ù…ÛŒ",
                                  "style": {
                                    "type": "custom",
                                    "fontSize": 16.0,
                                    "fontWeight": "w500"
                                  },
                                  "textDirection": "rtl",
                                  "type": "text"
                                },
                                {
                                  "child": {
                                    "type": "sizedBox"
                                  },
                                  "type": "expanded"
                                },
                                {
                                  "type": "visibility",
                                  "visible": "[[crAddTypePermanentSel]]",
                                  "child": {
                                    "decoration": {
                                      "color": "#20C4D8",
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "child": {
                                      "child": {
                                        "icon": "check",
                                        "iconType": "material",
                                        "size": 16.0,
                                        "color": "#FFFFFF",
                                        "type": "icon"
                                      },
                                      "type": "center"
                                    },
                                    "type": "container"
                                  },
                                  "replacement": {
                                    "decoration": {
                                      "border": {
                                        "color": "{{appColors.current.input.borderEnabled}}",
                                        "width": 1.0
                                      },
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "type": "container"
                                  }
                                }
                              ],
                              "type": "row"
                            },
                            "type": "container"
                          }
                        },
                        "onTap": {
                          "actionType": "setValue",
                          "values": [
                            {
                              "key": "crAddTypePermanentSel",
                              "value": true
                            },
                            {
                              "key": "crAddTypeCreditSel",
                              "value": false
                            }
                          ]
                        },
                        "type": "gestureDetector"
                      },
                      {
                        "height": 10.0,
                        "type": "sizedBox"
                      },
                      {
                        "child": {
                          "type": "visibility",
                          "visible": "[[crAddTypeCreditSel]]",
                          "child": {
                            "decoration": {
                              "border": {
                                "color": "#20C4D8",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "padding": {
                                "left": 12.0,
                                "top": 14.0,
                                "right": 12.0,
                                "bottom": 14.0
                              },
                              "decoration": {
                                "border": {
                                  "color": "{{appColors.current.input.borderEnabled}}",
                                  "width": 1.0
                                },
                                "borderRadius": {
                                  "topLeft": 10.0,
                                  "topRight": 10.0,
                                  "bottomLeft": 10.0,
                                  "bottomRight": 10.0
                                }
                              },
                              "child": {
                                "textDirection": "rtl",
                                "children": [
                                  {
                                    "data": "Ø§Ø¹ØªØ¨Ø§Ø±ÛŒ",
                                    "style": {
                                      "type": "custom",
                                      "fontSize": 16.0,
                                      "fontWeight": "w500"
                                    },
                                    "textDirection": "rtl",
                                    "type": "text"
                                  },
                                  {
                                    "child": {
                                      "type": "sizedBox"
                                    },
                                    "type": "expanded"
                                  },
                                  {
                                    "type": "visibility",
                                    "visible": "[[crAddTypeCreditSel]]",
                                    "child": {
                                      "decoration": {
                                        "color": "#20C4D8",
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "child": {
                                        "child": {
                                          "icon": "check",
                                          "iconType": "material",
                                          "size": 16.0,
                                          "color": "#FFFFFF",
                                          "type": "icon"
                                        },
                                        "type": "center"
                                      },
                                      "type": "container"
                                    },
                                    "replacement": {
                                      "decoration": {
                                        "border": {
                                          "color": "{{appColors.current.input.borderEnabled}}",
                                          "width": 1.0
                                        },
                                        "shape": "circle"
                                      },
                                      "width": 22.0,
                                      "height": 22.0,
                                      "type": "container"
                                    }
                                  }
                                ],
                                "type": "row"
                              },
                              "type": "container"
                            },
                            "type": "container"
                          },
                          "replacement": {
                            "padding": {
                              "left": 12.0,
                              "top": 14.0,
                              "right": 12.0,
                              "bottom": 14.0
                            },
                            "decoration": {
                              "border": {
                                "color": "{{appColors.current.input.borderEnabled}}",
                                "width": 1.0
                              },
                              "borderRadius": {
                                "topLeft": 10.0,
                                "topRight": 10.0,
                                "bottomLeft": 10.0,
                                "bottomRight": 10.0
                              }
                            },
                            "child": {
                              "textDirection": "rtl",
                              "children": [
                                {
                                  "data": "Ø§Ø¹ØªØ¨Ø§Ø±ÛŒ",
                                  "style": {
                                    "type": "custom",
                                    "fontSize": 16.0,
                                    "fontWeight": "w500"
                                  },
                                  "textDirection": "rtl",
                                  "type": "text"
                                },
                                {
                                  "child": {
                                    "type": "sizedBox"
                                  },
                                  "type": "expanded"
                                },
                                {
                                  "type": "visibility",
                                  "visible": "[[crAddTypeCreditSel]]",
                                  "child": {
                                    "decoration": {
                                      "color": "#20C4D8",
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "child": {
                                      "child": {
                                        "icon": "check",
                                        "iconType": "material",
                                        "size": 16.0,
                                        "color": "#FFFFFF",
                                        "type": "icon"
                                      },
                                      "type": "center"
                                    },
                                    "type": "container"
                                  },
                                  "replacement": {
                                    "decoration": {
                                      "border": {
                                        "color": "{{appColors.current.input.borderEnabled}}",
                                        "width": 1.0
                                      },
                                      "shape": "circle"
                                    },
                                    "width": 22.0,
                                    "height": 22.0,
                                    "type": "container"
                                  }
                                }
                              ],
                              "type": "row"
                            },
                            "type": "container"
                          }
                        },
                        "onTap": {
                          "actionType": "setValue",
                          "values": [
                            {
                              "key": "crAddTypePermanentSel",
                              "value": false
                            },
                            {
                              "key": "crAddTypeCreditSel",
                              "value": true
                            }
                          ]
                        },
                        "type": "gestureDetector"
                      },
                      {
                        "height": 14.0,
                        "type": "sizedBox"
                      },
                      {
                        "onPressed": {
                          "actionType": "sequence",
                          "actions": [
                            {
                              "actionType": "setValue",
                              "values": [
                                {
                                  "key": "crSimHasItems",
                                  "value": true
                                },
                                {
                                  "key": "crSimShowDuplicateBanner",
                                  "value": false
                                },
                                {
                                  "key": "crSim3Visible",
                                  "value": true
                                },
                                {
                                  "key": "crSim1Selected",
                                  "value": false
                                },
                                {
                                  "key": "crSim2Selected",
                                  "value": false
                                },
                                {
                                  "key": "crSim3Selected",
                                  "value": true
                                },
                                {
                                  "key": "crShowSimActionSheet",
                                  "value": false
                                },
                                {
                                  "key": "crShowSimEditSheet",
                                  "value": false
                                },
                                {
                                  "key": "crShowSimDeleteDialog",
                                  "value": false
                                },
                                {
                                  "key": "crAddShowOperatorSheet",
                                  "value": false
                                },
                                {
                                  "key": "crAddShowSimTypeSheet",
                                  "value": false
                                },
                                {
                                  "key": "crSim3Operator",
                                  "value": "Ù‡Ù…Ø±Ø§Ù‡ Ø§ÙˆÙ„",
                                  "condition": "crAddOpMciSel"
                                },
                                {
                                  "key": "crSim3Logo",
                                  "value": "assets/icons/ic_hamrah_aval.svg",
                                  "condition": "crAddOpMciSel"
                                },
                                {
                                  "key": "crSim3Operator",
                                  "value": "Ø§ÛŒØ±Ø§Ù†Ø³Ù„",
                                  "condition": "crAddOpIrancellSel"
                                },
                                {
                                  "key": "crSim3Logo",
                                  "value": "assets/icons/ic_irancell.svg",
                                  "condition": "crAddOpIrancellSel"
                                },
                                {
                                  "key": "crSim3Operator",
                                  "value": "Ø±Ø§ÛŒØªÙ„",
                                  "condition": "crAddOpRightelSel"
                                },
                                {
                                  "key": "crSim3Logo",
                                  "value": "assets/icons/ic_rightel.svg",
                                  "condition": "crAddOpRightelSel"
                                },
                                {
                                  "key": "crActiveSimOperator",
                                  "value": "Ù‡Ù…Ø±Ø§Ù‡ Ø§ÙˆÙ„",
                                  "condition": "crAddOpMciSel"
                                },
                                {
                                  "key": "crActiveSimLogo",
                                  "value": "assets/icons/ic_hamrah_aval.svg",
                                  "condition": "crAddOpMciSel"
                                },
                                {
                                  "key": "crActiveSimOperator",
                                  "value": "Ø§ÛŒØ±Ø§Ù†Ø³Ù„",
                                  "condition": "crAddOpIrancellSel"
                                },
                                {
                                  "key": "crActiveSimLogo",
                                  "value": "assets/icons/ic_irancell.svg",
                                  "condition": "crAddOpIrancellSel"
                                },
                                {
                                  "key": "crActiveSimOperator",
                                  "value": "Ø±Ø§ÛŒØªÙ„",
                                  "condition": "crAddOpRightelSel"
                                },
                                {
                                  "key": "crActiveSimLogo",
                                  "value": "assets/icons/ic_rightel.svg",
                                  "condition": "crAddOpRightelSel"
                                }
                              ]
                            },
                            {
                              "actionType": "setValue",
                              "key": "crSim3Number",
                              "value": "{{crAddEnteredPhone}}"
                            },
                            {
                              "actionType": "setValue",
                              "key": "crActiveSimNumber",
                              "value": "{{crAddEnteredPhone}}"
                            },
                            {
                              "routeName": "charge_real_package_list",
                              "navigationStyle": "push",
                              "actionType": "navigate"
                            }
                          ]
                        },
                        "style": {
                          "foregroundColor": "{{appColors.current.primary.onPrimary}}",
                          "backgroundColor": "{{appColors.current.primary.color}}",
                          "elevation": 0.0,
                          "fixedSize": {
                            "width": 999999.0,
                            "height": 56.0
                          },
                          "shape": {
                            "type": "roundedRectangleBorder",
                            "borderRadius": {
                              "topLeft": 10.0,
                              "topRight": 10.0,
                              "bottomLeft": 10.0,
                              "bottomRight": 10.0
                            }
                          }
                        },
                        "child": {
                          "data": "ØªØ§ÛŒÛŒØ¯ Ùˆ Ø§Ø¯Ø§Ù…Ù‡",
                          "style": {
                            "type": "custom",
                            "color": "{{appColors.current.primary.onPrimary}}",
                            "fontSize": 16.0,
                            "fontWeight": "w700"
                          },
                          "textDirection": "rtl",
                          "type": "text"
                        },
                        "type": "filledButton"
                      }
                    ],
                    "type": "column"
                  },
                  "type": "container"
                },
                "type": "align"
              }
            ],
            "type": "stack"
          },
          "replacement": {
            "type": "sizedBox"
          }
        }
      ],
      "type": "stack"
    },
    "type": "scaffold"
  }
}
```
