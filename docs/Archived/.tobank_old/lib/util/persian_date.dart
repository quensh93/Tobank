...
class PersianDate {
  ...
  PersianDate.pDate({String? defualtFormat, String? gregorian}) {
    ...
    if (gregorian != null) {
      ...
      final List list = gregorianToJalali(now.year, now.month, now.day);
  ...
  parse(String formattedString, [String? separator]) {
    ...
    if (separator == null) {
      final List parseList = gregorianToJalali(parse.year, parse.month, parse.day);
  ...
  bool get isHoliday {
    if (weekday == 5) {
      return true;
    } else if (shamsiHoliday.contains('${_digits(month, 2)}${_digits(day, 2)}')) {
      return true;
    } else {
      return false;
    }
  ...
  _toFormat(String format) {
    ...
    if (newFormat.contains(AM)) {
      ...
      newFormat = newFormat.replaceFirst(AM, hour! < 12 ? locale.pm : locale.am);
    }
    if (newFormat.contains(am)) {
  ...
  parseToFormat(String parseDate, [String? format]) {
    ...
    final jParse = gregorianToJalali(parse.year, parse.month, parse.day);
    format ??= _defualtVal;
    ...
    if (newFormat.contains(AM)) {//locale
      final locale = AppLocalizations.of(Get.context!)!;
      newFormat = newFormat.replaceFirst(AM, parse.hour < 12 ? locale.am : locale.pm);
    }
    if (newFormat.contains(am)) {
