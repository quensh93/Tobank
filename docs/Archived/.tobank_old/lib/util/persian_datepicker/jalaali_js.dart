...
class Jalaali {
  ...
  int get julianDayNumber {
    ...
    return Gregorian(r.gy, 3, r.march).julianDayNumber + (month! - 1) * 31 - _div(month!, 7) * (month! - 7) + day! - 1;
  }
  ...
  bool isValid() {
    return year! >= -61 && year! <= 3177 && month! >= 1 && month! <= 12 && day! >= 1 && day! <= monthLength;
  }
...
class Gregorian {
  ...
  int get julianDayNumber {
    final int d = _div((year! + _div(month - 8, 6) + 100100) * 1461, 4) +
        _div(153 * _mod(month + 9, 12) + 2, 5) +
        day! -
        34840408;

    return d - _div(_div(year! + 100100 + _div(month - 8, 6), 100) * 3, 4) + 752;
  }
  ...
  factory Gregorian.fromJulianDayNumber(int julianDayNumber) {
    ...
    j = j + _div(_div(4 * julianDayNumber + 183187720, 146097) * 3, 4) * 4 - 3908;
  ...
  Gregorian.fromDateTime(DateTime dateTime) : this(dateTime.year, dateTime.month, dateTime.day);
...
String getGregorianMonthName(int month) {//locale
  final locale = AppLocalizations.of(Get.context!)!;
  switch (month) {
    case 1:
      return locale.january;
