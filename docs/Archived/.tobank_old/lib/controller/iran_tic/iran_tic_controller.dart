...
class IranTicController extends GetxController with WidgetsBindingObserver {
      ...
      if (state == AppLifecycleState.resumed) {
        Timer(Constants.duration200, () {
          webViewController?.loadUrl(
              urlRequest: URLRequest(url: WebUri.uri(Uri.parse('https://www.irantic.com/profile/order'))));
        });
        isInBrowser = false;
      }
    }
  ...
  String startedUtl() {
    return 'https://www.irantic.com/events';
  }
...
//locale
    ...
    if (uri.host.contains('irantic.com') && uri.path.contains('print')) {
      AppUtil.launchInBrowser(url: uri.toString());
      isInBrowser = true;
      return NavigationActionPolicy.CANCEL;
    } else if (uri.toString().contains('shaparak.ir')) {
      ...
      if (uri.host.contains('irantic.com')) {
