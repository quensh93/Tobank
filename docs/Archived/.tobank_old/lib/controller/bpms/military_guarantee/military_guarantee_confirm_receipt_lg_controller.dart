...
import '../../../model/bpms/military_guarantee/request/complete_confirm_receipt_lg_task_data.dart';
...
class MilitaryGuaranteeConfirmReceiptLGController extends GetxController {
  final Task task;
  ...
  MilitaryGuaranteeConfirmReceiptLGController({required this.task, required this.taskData});

  void validateReceiptLG({required bool isConfirmed}) {
    _completeDraftFileTaskRequest(isConfirmed);
  }

  /// Completes the draft file task by sending a request to the backend.
  void _completeDraftFileTaskRequest(bool isConfirmed) {
//locale
    ...
    final CompleteTaskRequest completeTaskRequest = CompleteTaskRequest(
      ...
      taskId: task.id!,
      taskData: CompleteConfirmReceiptLGTaskData(
        confirmReceipt: isConfirmed,
      ),
    );
