...
import '../../../model/bpms/military_guarantee/request/complete_employment_employee_task_data.dart';
...
class MilitaryGuaranteeEmploymentEmployeeController extends GetxController {
  final Task task;
  ...
  String? documentFileDescription_2;

  MilitaryGuaranteeEmploymentEmployeeController({required this.task, required this.taskData}) {
  ...
  void _getDataFromTaskData() {
    for (final formField in taskData) {
      if (formField.value?.subValue != null) {
        switch (formField.id) {
          case 'applicantEmploymentDocument':
            final documentValue = formField.value!.subValue! as Map<String, dynamic>;
            documentFileDescription_1 = documentValue['description'];
            break;
          case 'applicantDeductionSalaryDocument':
            final documentValue = formField.value!.subValue! as Map<String, dynamic>;
            documentFileDescription_2 = documentValue['description'];
            break;
        }
      }
    }
  ...
  void validateEmploymentEmployeePage() {
//locale
    final locale = AppLocalizations.of(Get.context!)!;
    AppUtil.hideKeyboard(Get.context!);
    if (_documentFileUUID_1 != null && _documentFileUUID_2 != null) {
      _submitEmploymentEmployeeRequest();
    } else {
      SnackBarUtil.showInfoSnackBar(
        locale.select_employment_documents,
      );
    }
  }

  /// Submits the employment employee request by sending data to the backend.
  void _submitEmploymentEmployeeRequest() {
//locale
    final locale = AppLocalizations.of(Get.context!)!;
    isLoading = true;
    update();
    ...
    final CompleteTaskRequest completeTaskRequest = CompleteTaskRequest(
      ...
      taskData: CompleteEmploymentEmployeeTaskData(
        applicantEmploymentDocument: DocumentFile(
            value: DocumentFileValue(
                id: _documentFileUUID_1, status: null, title: 'EmploymentDocument', description: null)),
        applicantDeductionSalaryDocument: DocumentFile(
            value: DocumentFileValue(
                id: _documentFileUUID_2, status: null, title: 'DeductionSalaryDocument', description: null)),
      ),
    );
