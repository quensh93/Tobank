...
import '../../../model/bpms/children_loan/request/complete_customer_collateral_info_task_data.dart';
...
import '../../../ui/bpms/children_loan_procedure/children_loan_add_warranty/warranty_view/customer_cheque_warranty_widget.dart';
import '../../../ui/bpms/children_loan_procedure/children_loan_add_warranty/warranty_view/customer_demand_note_warranty_widget.dart';
import '../../../ui/bpms/children_loan_procedure/children_loan_add_warranty/warranty_view/customer_salary_warranty_widget.dart';
import '../../../ui/bpms/children_loan_procedure/children_loan_add_warranty/warranty_view/support_cheque_warranty_widget.dart';
import '../../../ui/bpms/children_loan_procedure/children_loan_add_warranty/warranty_view/support_salary_warranty_widget.dart';
...
class ChildrenLoanProcedureAddWarrantyController extends GetxController {
  final Task task;
  ...
  ChildrenLoanProcedureAddWarrantyController({required this.task, required this.taskData}) {
...
//locale
    ...
    if (selectedWarrantyData != null) {
      if (selectedWarrantyData!.id == 0) {
        if (collaterals[indexPage].collateralDocumentId != null) {
          ...
          isValid = false;
          SnackBarUtil.showInfoSnackBar(locale.please_select_cheque);
        }
        ...
        if (collaterals[indexPage].collateralDocumentId != null) {
          ...
          isValid = false;
          SnackBarUtil.showInfoSnackBar(locale.please_select_applicant_salary_certificate);
        }
      } else if (selectedWarrantyData!.id == 3) {
        if (collaterals[indexPage].collateralDocumentId != null) {
        } else {
        ...
        if (collaterals[indexPage].collateralDocumentId != null) {
          ...
          isValid = false;
          SnackBarUtil.showInfoSnackBar(locale.please_select_guarantor_salary_certificate);
        }
      }
      update();
      if (isValid) {
      ...
      SnackBarUtil.showInfoSnackBar(locale.please_select_guarantee_type);
    }
  }
  ...
  /// Completes a customer collateral information task in a workflow.
...
//locale
    ...
    isLoading = true;
    update();
    BPMSServices.completeTask(completeTaskRequest: completeTaskRequest).then((result) {
      isLoading = false;
      update();
  ...
  /// Selects and processes an image for a document.
  ///
  /// This function allows the user to select an image from either the camera or gallery./// It then crops the image and checks its size.
  /// If the image is too large, it displays an error message.
...
//locale
  ...
  void _uploadDocumentRequest(File image, int documentId, String customerNumber) { //locale
    final locale = AppLocalizations.of(Get.context!)!;
