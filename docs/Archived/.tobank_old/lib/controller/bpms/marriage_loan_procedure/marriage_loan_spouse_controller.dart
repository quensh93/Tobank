...
import '../../../model/bpms/marriage_loan/request/complete_spouse_info_task_data.dart';
...
class MarriageLoanProcedureSpouseController extends GetxController {
  final Task task;
  ...
  TextEditingController nationalCodeSpouseController = TextEditingController();

  bool isSpouseNationalCodeValid = true;

  TextEditingController birthdayDateSpouseController = TextEditingController();

  bool isSpouseDateValid = true;

  TextEditingController spouseNationalCodeTrackingNumberController = TextEditingController();

  bool isSpouseNationalCodeTrackingNumberValid = true;

  bool noSpouseNationalCard = false;
  ...
  MarriageLoanProcedureSpouseController({required this.task, required this.taskData});
...
//locale
    ...
    showModalBottomSheet(
        ...
        builder: (context) {
          return DateSelectorBottomSheet(
            ...
            callback: () {
              birthdayDateSpouseController.text = dateJalaliString ?? '';
              update();
              Get.back();
            },
          );
  ...
  void validateSpouseBirthCertificate() {
//locale
    final locale = AppLocalizations.of(Get.context!)!;
    AppUtil.hideKeyboard(Get.context!);
    bool isValid = true;
    if (_documentFileUUID_1 != null && _documentFileUUID_2 != null && _documentFileUUID_3 != null) {
      if (noSpouseNationalCard) {
        if (spouseNationalCodeTrackingNumberController.text.trim().length >= 8 &&
            spouseNationalCodeTrackingNumberController.text.trim().length <= 12) {
          isSpouseNationalCodeTrackingNumberValid = true;
        } else {
          isSpouseNationalCodeTrackingNumberValid = false;
          isValid = false;
        }
        update();
      } else {
        if (_documentFileUUID_4 != null && _documentFileUUID_5 != null) {
        } else {
          isValid = false;
    ...
    if (isValid) {
      _completeSpouseInfoRequest();
    }
  }

  void setNoSpouseNationalCard(bool value) {
    noSpouseNationalCard = value;
    update();
  }

  /// Sends a request to check the personal information of the spouse.
  void _checkPersonalInfoSpouseRequest() {
//locale
    final locale = AppLocalizations.of(Get.context!)!;
    final CheckPersonalInfoRequestData checkPersonalInfoRequestData = CheckPersonalInfoRequestData(
      trackingNumber: const Uuid().v4(),
      nationalCode: nationalCodeSpouseController.text,
      birthDate: DateConverterUtil.getTimestampFromJalali(
          date: birthdayDateSpouseController.text, extendDuration: const Duration(hours: 2)),
  ...
  /// Validates the spouse's information
  /// and, if valid, proceeds to check the spouse's personal information.
  void validateSpouseCheck() {
    AppUtil.hideKeyboard(Get.context!);
    bool isValid = true;
    if (nationalCodeSpouseController.text.length == Constants.nationalCodeLength &&
        AppUtil.validateNationalCode(nationalCodeSpouseController.text)) {
      isSpouseNationalCodeValid = true;
    } else {
      isSpouseNationalCodeValid = false;
      isValid = false;
    }
    if (birthdayDateSpouseController.text.isNotEmpty) {
      isSpouseDateValid = true;
    } else {
      isValid = false;
      isSpouseDateValid = false;
    }
    update();
    if (isValid) {
      _checkPersonalInfoSpouseRequest();
    }
  }

  /// Completes the spouse information request by sending a request
  /// to the backend with the spouse's information.
  void _completeSpouseInfoRequest() {
//locale
    final locale = AppLocalizations.of(Get.context!)!;
    isLoading = true;
    update();
    ...
    final CompleteTaskRequest completeTaskRequest = CompleteTaskRequest(
      ...
      trackingNumber: const Uuid().v4(),
      taskId: task.id!,
      taskData: CompleteSpouseInfoTaskData(
        spouseBirthCertificateFirstDocument: DocumentFile(
        ...
        spouseBirthCertificateSecondDocument: DocumentFile(
          value: DocumentFileValue(
        ...
        spouseBirthCertificateThirdDocument: DocumentFile(
          value: DocumentFileValue(
        ...
        spouseBirthDate: DateConverterUtil.getTimestampFromJalali(
            date: birthdayDateSpouseController.text, extendDuration: const Duration(hours: 2)),
        spouseHasSmartNationalIdCard: !noSpouseNationalCard,
        spouseNationalId: nationalCodeSpouseController.text,
        spouseNationalIdTrackingNumber: noSpouseNationalCard ? spouseNationalCodeTrackingNumberController.text : null,
        spouseNationalIdFrontDocument: noSpouseNationalCard
            ? null
            : DocumentFile(
                value: DocumentFileValue(
        ...
        spouseNationalIdBackDocument: noSpouseNationalCard
            ? null
            : DocumentFile(
                value: DocumentFileValue(
