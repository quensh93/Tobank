...
import '../../../model/bpms/credit_card_facility/request/complete_guarantor_documents_task_data.dart';
...
class CreditCardGuaranteeDocumentController extends GetxController {
  final Task task;
  ...
  CreditCardGuaranteeDocumentController({required this.task, required this.taskData});
...
//locale
  ...
  /// Uploads a document to the server distinguished by it's [documentId]
  ...
  /// Completes the guarantor documents request by sending a request to the backend
  /// with the uploaded document IDs.
  ///
  /// If the request is successful, it navigates back to the previous screen and displays a success message.
  ...
  /// Validates if all required guarantor documents have been uploaded
  /// and, if so, proceeds with the document completion request.
  void validateGuaranteeDocuments() {
//locale
    final locale = AppLocalizations.of(Get.context!)!;
  ...
  /// If the request is successful, it navigates back to the previous screen and displays a success message.
  /// If the request fails, it displays an error message.
  void _completeGuarantorDocumentsRequest() {
//locale
    final locale = AppLocalizations.of(Get.context!)!;
    final CompleteTaskRequest completeTaskRequest = CompleteTaskRequest(
      ...
      taskData: CompleteGuarantorDocumentsTaskData(
        guarantorBirthCertificateMarriageDocumentId: _documentFileUUID_1!,
        guarantorBirthCertificateChildrenDocumentId: _documentFileUUID_2!,
        guarantorBirthCertificateDeathInfoDocumentId: _documentFileUUID_3!,
        guarantorEmploymentDocumentId: _documentFileUUID_4!,
      ),
    );
    isLoading = true;
    update();
