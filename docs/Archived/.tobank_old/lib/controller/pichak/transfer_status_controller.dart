...
class TransferStatusController extends GetxController {
  ...
  late TransferStatusInquiryResponse transferStatusInquiryResponse;
  TextEditingController chequeIdController = TextEditingController();
  bool isIdValid = true;

  int openBottomSheets = 0;

  /// Checks Shaparak payment data and either displays a confirmation bottomSheet
  /// or initiates a transfer status inquiry request.
  Future<void> transferStatusInquiry() async {
    ...
    if (storeShaparakPaymentModel == null ||
      ...
      _transferStatusInquiryRequest();
    }
  }

  // Sends a transfer status inquiry request and handles the response,
  // either navigating to the next page or displaying an error message.
  void _transferStatusInquiryRequest() {//locale
    final locale = AppLocalizations.of(Get.context!)!;
    final TransferStatusInquiryRequest transferStatusInquiryRequest = TransferStatusInquiryRequest();
    transferStatusInquiryRequest.chequeId = chequeIdController.text;

    isLoading = true;
    update();

    PichakServices.transferStatusInquiry(
      transferStatusInquiryRequest: transferStatusInquiryRequest,
    ).then((result) {
      isLoading = false;
      update();

      switch (result) {
        case Success(value: (final TransferStatusInquiryResponse response, int _)):
          storeShaparakPaymentCount();
          transferStatusInquiryResponse = response;
          update();
  ...
  /// Stores payment time and Mana ID in shared preferences for a specified duration.
