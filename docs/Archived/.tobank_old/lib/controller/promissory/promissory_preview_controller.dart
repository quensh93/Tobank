...
class PromissoryPreviewController extends GetxController {
  MainController mainController = Get.find();

  final Uint8List pdfData;
  final String? promissoryId;

  PromissoryPreviewController({required this.pdfData, this.promissoryId});


  Future<void> sharePromissoryPdf() async {
  ...
  /// Stores a PDF document to the device's storage.
  Future<void> storePdf() async {//locale
    final locale = AppLocalizations.of(Get.context!)!;
    if (Platform.isAndroid) {
      final bool isGranted = await PermissionHandler.storage.isGranted;
      if (isGranted) {
        await FileSaver.instance.saveAs(
            name: '${promissoryId ?? DateTime.now().millisecondsSinceEpoch}-MultiSigned',
            bytes: pdfData,
            ext: 'pdf',
        ...
        DialogUtil.showDialogMessage(
            ...
            description: locale.need_access_to_save_promisoory,
