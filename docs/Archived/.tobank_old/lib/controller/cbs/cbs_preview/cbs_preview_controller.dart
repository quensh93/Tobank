...
class CBSPreviewController extends GetxController {
  MainController mainController = Get.find();

  final Uint8List pdfData;
  final String? nationalCode;

  CBSPreviewController({required this.pdfData, this.nationalCode});
  ...
  Future<void> shareCBSPdf() async {
    ...
    if (kIsWeb) {
      ...
      FileUtil().shareFile(
        ...
        name:
            '${nationalCode ?? DateTime.now().millisecondsSinceEpoch}-CBS.pdf',
  ...
  void _showWebShareOptions(String url, String filename) {//locale
    final locale = AppLocalizations.of(Get.context!)!;
  ...
  Future<void> storePdf() async {
    if (kIsWeb) {
      ...
      await FileSaver.instance.saveFile(
        name: '${nationalCode ?? DateTime.now().millisecondsSinceEpoch}-CBS',
        bytes: pdfData,
        ext: 'pdf',
        mimeType: MimeType.pdf,
      );
    } else if (Platform.isAndroid) {
      // Your existing Android implementation
      ...
      if (Platform.isAndroid) {
        ...
        if (isGranted) {
          await FileSaver.instance.saveAs(
              name:
                  '${nationalCode ?? DateTime.now().millisecondsSinceEpoch}-CBS',
              bytes: pdfData,
              ext: 'pdf',
              mimeType: MimeType.pdf);
        } else {
          DialogUtil.showDialogMessage(
              ...
              positiveFunction: () async {
                Get.back();
                final isGranted =
                    await PermissionHandler.storage.handlePermission();
                if (isGranted) {
                  await FileSaver.instance.saveAs(
                      name:
                          '${nationalCode ?? DateTime.now().millisecondsSinceEpoch}-CBS',
