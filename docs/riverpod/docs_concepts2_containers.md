...
If Riverpod were to allow providers to store their own state, it would imply that _anything_ could read/write to that state. This means that it would be difficult to control how/when a state is modified.
...
Through ProviderContainer and ProviderScope, we can configure various app-wide aspects of Riverpod. For example:
  ...
  4. Support for Scoping providers. By storing the state of a provider inside a container, we can have the same provider resolve to a different state depending on where in the widget tree it is used. This feature is quite advanced and generally discouraged, but useful for performance optimizations.
[](https://github.com/rrousselGit/riverpod/edit/master/website/docs/concepts2/containers.mdx)
 Refs
  * Using a ProviderContainer for pure Dart applications
  * Using a ProviderScope for Flutter applications
  * Why store the state of providers inside a container?
