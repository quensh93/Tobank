...
class AuthenticationRealLoaderParser extends StacParser<Map<String, dynamic>> {
  ...
  @override
  String get type => 'authentication_loader';
  ...
  Widget parse(BuildContext context, Map<String, dynamic> model) {
    return const AuthenticationRealLoaderScreen();
  }
}

/// The Flutter widget that fetches and renders verify identity SDUI from API.
class AuthenticationRealLoaderScreen extends StatefulWidget {
  const AuthenticationRealLoaderScreen({super.key});

  @override
  State<AuthenticationRealLoaderScreen> createState() =>
      _AuthenticationRealLoaderScreenState();
}

class _AuthenticationRealLoaderScreenState
    extends State<AuthenticationRealLoaderScreen> {
  ...
  Future<void> _fetchSduiConfig() async {
    ...
    try {
      final sduiJson = await _configApiService.fetchSduiConfig(
        pathKey: 'ipaam.builder.form.form.authentication_intro',
        build: 1,
      );
