...
/// Dio interceptor that serves mock responses from local JSON asset files.
///
/// **Mock File Structure:**
...
class StacMockInterceptor extends Interceptor {
      ...
      if (path.startsWith('screens/')) {
        ...
        if (assetPath == null && path.contains('/')) {
          ...
          if (parts.length == 2) {
            ...
            // Pattern: promissory/sign/asset -> lib/stac/tobank/flows/promissory/api/GET_sign_asset.json
            final lastPart = parts.last;
            final filename2 = '${method}_$lastPart.json';
            final testPath =
                'lib/stac/tobank/flows/promissory/api/$filename2';
            try {
              await rootBundle.loadString(testPath);
              assetPath = testPath;
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    AppLogger.ec(
      LogCategory.network,
      'STAC Error: ${err.message} path: ${err.requestOptions.uri}',
    );
    if (err.response != null) {
      ...
      AppLogger.ec(LogCategory.network, '   Data: ${err.response?.data}');
    }
    handler.next(err);
  }
}
