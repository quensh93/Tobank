        ...
        try {
          ...
          if (path.startsWith('screens/')) {
            ...
            if (featureFolder != null) {
              ...
              final featureFolders = ['login', 'otp', 'menu', 'home', 'account', 'transfer', 'transactions', 'profile'];
            ...
            if (assetPath == null && path.contains('/')) {
              ...
              if (parts.length == 2) {
                ...
                final testPath = 'lib/stac/tobank/$feature/api/GET_$screen.json';
                try {
                  await rootBundle.loadString(testPath);
                  assetPath = testPath;
          ...
          if (assetPath == null) {
            AppLogger.w('⚠️ Mock interceptor: No asset path found for URL: ${options.uri}');
          ...
          try {
            AppLogger.d('🔍 Mock interceptor: Looking for file: $finalAssetPath');
            ...
            if (finalAssetPath.contains('GET_styles.json')) {
              final lightCount = RegExp(r'appColors\.light\.').allMatches(jsonString).length;
              final currentCount = RegExp(r'appColors\.current\.').allMatches(jsonString).length;
              ...
              if (lightCount > 0) {
                AppLogger.e('   ❌ ERROR: Found $lightCount appColors.light.* references!');
                AppLogger.e('   These should be appColors.current.* for theme awareness!');
              }
              if (currentCount == 0 && lightCount > 0) {
                AppLogger.e('   ❌ CRITICAL: No appColors.current.* references found!');
              }
            }
            ...
            if (isScreenJson) {
              AppLogger.d('✅ Mock interceptor: Returning screen JSON for $method $path');
              ...
              if (jsonData.containsKey(method) && jsonData[method] is Map<String, dynamic>) {
                final methodData = jsonData[method] as Map<String, dynamic>;
                if (methodData.containsKey('data') && methodData['data'] is Map<String, dynamic>) {
                  widgetJson = methodData['data'] as Map<String, dynamic>;
                  AppLogger.d('   📦 Extracted widget JSON from API wrapper (${method}.data)');
                }
              }
              
              // Debug: Check a sample variable before resolution
              final sampleVar = StacRegistry.instance.getValue('appStyles.text.pageTitle.fontSize');
              AppLogger.d('   🔍 Sample variable from registry: appStyles.text.pageTitle.fontSize');
              AppLogger.d('      Type: ${sampleVar.runtimeType}, Value: $sampleVar');
              
              // Pre-resolve variables to preserve types (especially for numeric style values)
              final resolvedJson = resolveVariablesPreservingTypes(widgetJson, StacRegistry.instance);
              
              // Debug: Check a sample resolved value
              if (resolvedJson is Map) {
                final sampleStyle = _getNestedValue(resolvedJson, ['appBar', 'title', 'style', 'fontSize']);
                if (sampleStyle != null) {
                  AppLogger.d('   🔍 Sample resolved fontSize type: ${sampleStyle.runtimeType}, value: $sampleStyle');
                }
                
                // Also check padding values
                final samplePadding = _getNestedValue(resolvedJson, ['body', 'children', '0', 'child', 'padding', 'left']);
                if (samplePadding != null) {
                  AppLogger.d('   🔍 Sample padding.left type: ${samplePadding.runtimeType}, value: $samplePadding');
                }
              }
              ...
              return handler.resolve(
                Response(
                  ...
                  statusCode: 200,
                  data: resolvedJson, // Screen JSONs with variables pre-resolved
                ),
              );
            }
            ...
            if (methodData != null) {
              ...
              AppLogger.d('✅ Mock interceptor: Returning response for $method $path (status: $statusCode)');
              ...
              AppLogger.w('⚠️ Mock interceptor: Method $method not found in $finalAssetPath');
            }
          } catch (e) {
            // File not found or error loading - let request continue normally
            // This allows real API calls if mock file doesn't exist
            AppLogger.w('⚠️ Mock file not found or error loading: $finalAssetPath - $e');
          }
        } catch (e) {
  ...
  if (json is String) {
    ...
    if (exactMatch != null) {
      ...
      if (variableName.startsWith('appStyles.')) {
        ...
        final isStyleObjectRef = parts.length == 1 ||
            (parts.length == 2 &&
                (parts.first == 'text' || parts.first == 'button' || parts.first == 'input'));
        if (isStyleObjectRef) {
      ...
      if (value != null) {
        ...
        if (variableName.contains('fontSize') || 
          ...
          AppLogger.d('      Retrieved type: ${value.runtimeType}, value: $value');
        }
        ...
        if (value is String && converted is! String) {
          AppLogger.d('   🔄 Converted $variableName from String to ${converted.runtimeType}: "$value" -> $converted');
        } else if (value is! String && converted is num) {
          AppLogger.d('   ✅ Preserved numeric type for $variableName: ${converted.runtimeType} = $converted');
        }
    ...
    return json.replaceAllMapped(RegExp(r'{{(.*?)}}'), (match) {
      ...
      if (value == null && variableName != null && variableName.startsWith('appData.')) {
    ...
    return json.map(
        (key, value) => MapEntry(key, resolveVariablesPreservingTypes(value, registry)));
  } else if (json is List) {
    return json.map((item) => resolveVariablesPreservingTypes(item, registry)).toList();
  }
  return json;
}
