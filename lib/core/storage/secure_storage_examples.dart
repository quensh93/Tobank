...
class SecureStorageExamples {
  ...
  static Future<void> authenticationFlowExample() async {
    try {
      ...
      final success = await SecureStorageService.write(
      ...
      if (success) {
        ...
        // Retrieve token
        final retrievedToken = await SecureStorageService.read(
          SecureStorageKeys.token,
        );
  ...
  static Future<void> userProfileExample() async {
    try {
      ...
      if (success) {
        ...
        final profileData = await SecureStorageService.readMultiple(
          profileKeys,
        );
  ...
  static Future<void> deviceTokenExample() async {
    try {
      ...
      final hasFcmToken = await SecureStorageService.containsKey(
        SecureStorageKeys.fcmToken,
      );
      final hasDeviceId = await SecureStorageService.containsKey(
    ...
    try {
      ...
      if (success) {
        ...
        if (decryptedData != null) {
          if (kDebugMode) {
            print(
              '✅ Data decrypted successfully: ${decryptedData.substring(0, 10)}...',
            );
          }
        }
      }
  ...
  static Future<void> expiringDataExample() async {
    try {
      ...
      final sessionData =
          'session_token_${DateTime.now().millisecondsSinceEpoch}';
    ...
    try {
      ...
      if (success) {
        ...
        if (kDebugMode) {
          ...
          print(
            '  Biometric: ${biometricEnabled == 'true' ? "Enabled" : "Disabled"}',
          );
          print(
            '  Auto Login: ${autoLoginEnabled == 'true' ? "Enabled" : "Disabled"}',
          );
        }
      }
    } catch (e) {
  ...
  static Future<void> logoutCleanupExample() async {
    try {
      ...
      if (success) {
        ...
        final isLogin = await SecureStorageService.read(
          SecureStorageKeys.isLogin,
        );
        ...
        if (kDebugMode) {
          ...
          print(
            '  Login Status: ${isLogin == null ? "✅ Cleared" : "❌ Still exists"}',
          );
        }
      }
  ...
  static Future<void> storageMonitoringExample() async {
    try {
      ...
      if (kDebugMode) {
        print(
          '🔍 Storage Validation: ${isValid ? "✅ Healthy" : "❌ Issues detected"}',
        );
      }
