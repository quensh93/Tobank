...
import '../../../../../core/stac/utils/registry_notifier.dart';

/// Promissory Real Flow - Deposits Selection Page
///
/// Fetches deposits from the real API and displays them for selection.
/// All UI is inlined here (no temp folder, no extra content parser).
...
class _PromissoryRealDepositsScreenState
  ...
  void _requestDeposits() {
    if (_requested) return;
    _requested = true;

    WidgetsBinding.instance.addPostFrameCallback((_) {
      final action = _buildFetchDepositsAction();
      Stac.onCallFromJson(action, context);
    });
  }

  Map<String, dynamic> _buildFetchDepositsAction() {
          ...
          if (deposits == null) {
            stacWidget = _buildErrorScreen(
              message: errorMessage ??
                  'خطا در بارگذاری اطلاعات سپرده‌ها',
            );
          } else {
            stacWidget = _buildDepositPageWithData(deposits);
          }
        }
...
StacWidget _buildErrorScreen({required String message}) {
...
StacWidget _buildDepositPageWithData(List<Map<String, String>> deposits) {
  if (deposits.isEmpty) {
