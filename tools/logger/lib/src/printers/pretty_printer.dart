...
class PrettyPrinter extends LogPrinter {
  ...
  // Borders are not used - we use simple space-based indentation instead
  // ignore: unused_field
  ...
  PrettyPrinter({
    ...
    // Border initialization removed - we use simple space-based indentation instead
    // No decorative borders are used in the output
  ...
  String stringifyMessage(dynamic message) {
    ...
    if (finalMessage is Map || finalMessage is Iterable) {
      ...
      result = encoder.convert(finalMessage);
    } else {
      result = finalMessage.toString();
    }

    // Apply truncation if enabled and strict length check passes.
    // We skip if message was originally a String, assuming Logger.log already handled it.
    if (message is! String &&
        Logger.truncateLogs &&
        Logger.maxLogLength > 0 &&
        result.length > Logger.maxLogLength) {
      return '${result.substring(0, Logger.maxLogLength)}... (truncated ${result.length - Logger.maxLogLength} chars)';
    }
  ...
  List<String> _formatAndPrint(
    ...
    List<String> buffer = [];
    // Use spaces instead of vertical line character for indentation
    var indent = '  '; // Two spaces for indentation
    var color = _getLevelColor(level);

    // Extract just the absolute time (remove relative time part if present)
    String? cleanTime;
    if (time != null) {
      // Remove the relative time part like "(+0:00:00.096625)"
      // Format: "HH:MM:SS.mmm" or "HH:MM:SS.mmm (+0:00:00.096625)"
      var timeMatch = RegExp(r'^(\d{2}:\d{2}:\d{2}\.\d+)').firstMatch(time);
      if (timeMatch != null) {
        cleanTime = timeMatch.group(1);
      } else {
        cleanTime = time;
      }
    }
    ...
    // Format: emoji - time - message (all on one line, no leading indent)
    for (var line in message.split('\n')) {
      if (cleanTime != null) {
        buffer.add(color('$emoji - $cleanTime - $line'));
      } else {
